...
pub const DIRECTORY: &str = ".kero";

const PROJECT: &str = r#"schema = "kero/project/v1"
id = "kero.project"
kind = "project"
...
const GLOBAL: &str = r#"schema = "kero/environment/v1"
id = "kero.global"
kind = "global"
...
pub enum DiscoveryError {
    #[error("kero.discovery.io: {0}")]
    Io(#[from] std::io::Error),
    #[error("kero.discovery.toml: {0}")]
    Toml(#[from] toml::de::Error),
    #[error("kero.discovery.encode: {0}")]
    Encode(#[from] toml::ser::Error),
    #[error("kero.discovery.home-unavailable")]
    HomeUnavailable,
    #[error("kero.discovery.global-unconfigured: run `kero global init`")]
    GlobalUnconfigured,
    #[error("kero.discovery.project-not-found: {0}")]
    ProjectNotFound(PathBuf),
    #[error("kero.discovery.identity-mismatch: expected {expected}, found {actual} at {path}")]
    IdentityMismatch {
        expected: String,
        actual: String,
        path: PathBuf,
...
pub fn initialize(root: &Path) -> Result<InitializedLayout, LayoutError> {
    ...
    let mut result = InitializedLayout {
        schema: "kero/initialized-layout/v1".into(),
        root: directory.clone(),
        created: Vec::new(),
        existing: Vec::new(),
    };
    create_file(&directory.join("kero.toml"), PROJECT, &mut result)?;
    create_file(
...
pub fn initialize_global(root: &Path) -> Result<InitializedLayout, LayoutError> {
    ...
    let mut result = InitializedLayout {
        schema: "kero/initialized-global-layout/v1".into(),
        root: root.to_path_buf(),
        created: Vec::new(),
        existing: Vec::new(),
    };
    create_file(&root.join("kero.toml"), GLOBAL, &mut result)?;
...
pub fn config_path() -> Result<PathBuf, DiscoveryError> {
    if let Some(path) = env::var_os("KERO_CONFIG_HOME") {
        return Ok(PathBuf::from(path).join("config.toml"));
    }
    if let Some(path) = env::var_os("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(path).join("kero/config.toml"));
    }
    Ok(home()?.join(".config/kero/config.toml"))
}
...
pub fn configure_global(root: &Path) -> Result<PathBuf, DiscoveryError> {
    ...
    let content = toml::to_string_pretty(&UserConfig {
        schema: "kero/config/v1".into(),
        global: GlobalLocator {
            root: root.to_path_buf(),
            required_id: "kero.global".into(),
        },
    })?;
...
pub fn read_identity(root: &Path) -> Result<KeroIdentity, DiscoveryError> {
    let content = fs::read_to_string(root.join("kero.toml"))?;
...
) -> Result<(PathBuf, KeroIdentity), DiscoveryError> {
...
pub fn discover_global(explicit: Option<&Path>) -> Result<(PathBuf, KeroIdentity), DiscoveryError> {
    if let Some(root) = explicit {
        return require_identity(root.to_path_buf(), "global", None);
    }
    if let Some(root) = env::var_os("KERO_GLOBAL_ROOT") {
    ...
    let fallback = home()?.join(".kero");
    if fallback.join("kero.toml").is_file() {
        return require_identity(fallback, "global", None);
    }
    Err(DiscoveryError::GlobalUnconfigured)
}

pub fn discover_project(start: &Path) -> Result<(PathBuf, KeroIdentity), DiscoveryError> {
    ...
    loop {
        ...
        if candidate.join("kero.toml").is_file() {
...
mod tests {
    ...
    fn initialization_adds_only_one_consumer_root_and_never_overwrites() {
        ...
        assert!(root.path().join(".kero/state").is_dir());
        assert_eq!(first.created.len(), 6);
        assert!(root.path().join(".kero/knowledge/shared").is_dir());
        assert!(root.path().join(".kero/knowledge/local").is_dir());
    ...
    fn global_and_project_discovery_use_separate_channels() {
        ...
        assert_eq!(project_path, repository.join(".kero"));
