//! Exact no-network Bubblewrap command boundary for Linux.
...
pub fn run(binary: impl Into<PathBuf>, spec: &CommandSpec) -> Result<Vec<u8>, BubblewrapError> {
    ...
    loop {
        if let Some(status) = child.try_wait()? {
            ...
            if !status.success() {
                return Err(BubblewrapError::Nonzero);
            }
            return Ok(bytes);
        }
...
mod tests {
    ...
    fn runs_a_no_network_smoke_command_when_bubblewrap_is_available() {
        ...
        let spec = CommandSpec {
            argv: vec!["/bin/echo".into(), "kero-bwrap-smoke".into()],
            working_directory: "/".into(),
            environment: vec![],
            timeout: Duration::from_secs(5),
            output_limit: 128,
        };
        assert_eq!(run(binary, &spec).unwrap(), b"kero-bwrap-smoke\n");
    }

    #[test]
    #[cfg(unix)]
    fn nonzero_sandbox_exit_is_not_reported_as_success() {
        let directory = tempdir().unwrap();
        let fake_bwrap = directory.path().join("bwrap-fail");
